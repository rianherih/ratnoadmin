...
if($kode_users == "semua")
{
	$sel_bbm = mysql_query("select a.*, 
		...
		c.nomor_barcode, 
		d.nama_users,
		e.waktu_estimasi from tb_history a, 
		tb_kerjaan b, tb_barcode c ,tb_users d,tb_listkerjaan e
		where waktu_mulai like '$bln%' 
		and a.no_kerjaan = b.no_kerjaan 
		and b.id_barcode = c.id_barcode 
		and d.kode_users = a.kode_users
		and e.id_listkerjaan = b.id_listkerjaan", koneksi_db());
}
else
{
	$sel_bbm = mysql_query("select a.*, 
		...
		and b.id_barcode = c.id_barcode 
		and d.kode_users = a.kode_users
		and e.id_listkerjaan = b.id_listkerjaan 
...
while($tpl_bbm = mysql_fetch_array($sel_bbm))
{
	...
	echo"<td>".$tpl_bbm['waktu_estimasi']." jam</td>";
